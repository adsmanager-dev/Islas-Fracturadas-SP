...
`config/` conserva datos declarativos de campaña separados de las clases de motor de `cfg/`. En M3, `config/sectors.hpp` declara 5 regiones, 9 sectores y 9 conexiones del vertical slice; `IF_fnc_configLoad` los materializa en `IF_config` y `IF_fnc_configValidate` comprueba IDs, membresías y referencias. Las coordenadas y radios de configuración aún usan arrays vacíos o `-1` y conservan `POR_CALIBRAR`. `mission.sqm` contiene tres centros validados en la pasada 1 —Neri–Panochori, Agios Dionysios y Lakka—, todavía sin migrar al contrato declarativo; esta evidencia física parcial no promueve sectores completos ni el gate M3 a `VALIDADO_3DEN`/`APROBADO`.
