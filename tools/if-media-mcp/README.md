...
- Escritura de `mission.sqm` (`arma_sqm_patch`): **excepción registrada en `AGENTS.md` el 2026-08-08** a petición explícita del usuario. Cambia SOLO `position[]`/`angles[]` de una entidad existente (localizada por su `id` nativo, único en todo el archivo) mediante un **parche quirúrgico de texto** (`scripts/sqm_patch.py`) — no un parseo-completo→regenerar-completo, que reformatearía el archivo entero (medido: 2.4× más grande, cada línea "cambiada" en un diff, aunque los datos fueran idénticos). Verificado con `diff` real: de 28643 líneas del `mission.sqm` de Antistasi, exactamente 1 cambia al mover una entidad. No añade ni borra entidades todavía. Antes de tocar el archivo real: backup automático con timestamp en `production/media/drafts/mission_sqm_backups/`, validación por round-trip (mismo nº de entidades, cero cambios en entidades ajenas) y confirmación explícita (`confirmation: "PATCH_MISSION_SQM_APPROVED"`). Si el archivo original estaba binarizado, el resultado queda en texto plano (equivalente a activar "Binarize the Scenario File = OFF" en Eden) — **abre y comprueba la misión en 3DEN/Arma 3 después de cualquier escritura**; la herramienta no sustituye esa verificación.
