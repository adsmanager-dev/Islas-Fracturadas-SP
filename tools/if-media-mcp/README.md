...
- Añadir entidades a `mission.sqm` (`arma_sqm_add_object`): misma excepción de `AGENTS.md`. Añade SOLO `dataType="Object"` como último elemento del bloque `Entities` raíz (hijo directo de `Mission`) — nunca dentro de un Group/Layer existente, ni otros `dataType` (Marker/Logic/Layer/Group tienen su propia forma de serialización, no soportada aún). Investigación previa (2026-08-08) confirmó en 140 bloques `Entities` reales que los índices `ItemN` son siempre contiguos 0..N-1, por eso añadir al final es seguro; borrar cualquier entidad que no sea la última exigiría renumerar todas las posteriores. Asigna un `id` nuevo sin colisión (máximo existente + 1) y mantiene sincronizados los tres lugares que 3DEN mantiene coherentes entre sí: `items=N;` del bloque raíz, el nuevo `class ItemN` insertado, y `class EditorData { class ItemIDProvider { nextID=...; }; }` si existe (verificado: en los mapas reales probados, `nextID` es siempre `id máximo + 1`). Copia la indentación real del archivo (tabs o espacios, detectada de la última entidad hermana) para mantener el diff legible. Verificado con `diff` real contra una copia de `mission.sqm` de Islas Fracturadas y otra de `AI_REFERENCES/A3-Antistasi`: solo cambian `nextID`, `items=` y el bloque nuevo insertado, cero entidades existentes afectadas. Mismo flujo de seguridad que `arma_sqm_patch` (backup, round-trip, confirmación explícita, verificación obligatoria en 3DEN/Arma 3 después).
- Borrar entidades de `mission.sqm` (`arma_sqm_delete_entity`): misma excepción de `AGENTS.md`. Simétrico de `arma_sqm_add_object` — borra SOLO la entidad que sea la ÚLTIMA del bloque `Entities` raíz (rechaza el resto sin tocar el archivo, incluida la única entidad restante si el bloque quedaría vacío: sin una hermana anterior de referencia no hay un punto de anclaje seguro, el mismo límite que `arma_sqm_add_object` tiene al revés). Deliberadamente NO decrementa `nextID` — no hay evidencia de que 3DEN reutilice IDs liberados por un borrado, así que dejarlo intacto (como mucho "adelantado" en uno) es el lado seguro. Verificado con una prueba de ida y vuelta añadir+borrar contra una copia real de `mission.sqm` de Antistasi: el resultado es **byte-idéntico al original salvo `nextID`**. Encontró y corrigió un bug real durante esa misma prueba: anclar el borrado en el inicio de línea de la propia entidad (en vez de en el final de la entidad anterior) dejaba una línea en blanco huérfana — el test unitario original solo comparaba estructura parseada, no bytes, y no lo detectó hasta la prueba contra el archivo real de ~470 KB. Mismo flujo de seguridad que las otras herramientas `arma_sqm_*`.
